...
local function destroy_notification_root()
	if NotificationState.Gui then
		NotificationState.Gui:Destroy()
	end

	NotificationState.Gui = nil
	NotificationState.Holder = nil
	NotificationState.Count = 0
end

local function ensure_notification_root()
	if NotificationState.Gui
		and NotificationState.Gui.Parent
		and NotificationState.Holder
		and NotificationState.Holder.Parent
	then
		return NotificationState.Gui, NotificationState.Holder
	end

	destroy_notification_root()

	local gui = Instance.new("ScreenGui")
	gui.Name = "ReverseNotifications"
	gui.ResetOnSpawn = false
	gui.IgnoreGuiInset = true
	gui.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
	...
	holder.Position = UDim2.new(1, -332, 0, 0)
...
local function create_notification_card(titleText, bodyText, durationValue)
	local _, holder = ensure_notification_root()
	NotificationState.Count = NotificationState.Count + 1

	local wrapper = Instance.new("Frame")
	wrapper.Name = "Notification_" .. tostring(NotificationState.Count)
	wrapper.Size = UDim2.new(1, 0, 0, 72)
	wrapper.BackgroundTransparency = 1
	wrapper.ClipsDescendants = false
	wrapper.LayoutOrder = NotificationState.Count
	wrapper.Parent = holder

	local card = Instance.new("Frame")
	card.Size = UDim2.new(1, 0, 1, 0)
	card.Position = UDim2.new(1, 18, 0, 0)
	card.BackgroundColor3 = Color3.fromRGB(14, 16, 28)
	...
	cardStroke.Color = Color3.fromRGB(50, 80, 160)
	...
	accentBar.Size = UDim2.new(0, 3, 0.72, 0)
	...
	iconShell.Size = UDim2.new(0, 32, 0, 32)
	iconShell.AnchorPoint = Vector2.new(0, 0.5)
	iconShell.Position = UDim2.new(0, 18, 0.5, 0)
	iconShell.BackgroundColor3 = Library.Theme.Accent
	iconShell.BackgroundTransparency = 0.78
	...
	iconLabel.TextColor3 = Library.Theme.Text
	...
	title.Text = tostring(titleText or "Notification")
	title.Parent = card
	...
	body.TextSize = 12
	body.TextColor3 = Library.Theme.TextDim
	...
	progressBg.BackgroundColor3 = Color3.fromRGB(30, 40, 70)
	...
	task.spawn(function()
		local duration = math.max(1, tonumber(durationValue) or 5)
		...
		TweenService:Create(progressFill, TweenInfo.new(math.max(0.2, duration - 0.25), Enum.EasingStyle.Linear, Enum.EasingDirection.In), {
		...
		TweenService:Create(cardStroke, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
			Transparency = 1,
		}):Play()

		TweenService:Create(glow, TweenInfo.new(0.3, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {
			BackgroundTransparency = 1,
		}):Play()
...
function Library:Notification(title, text, duration)
	local ok = pcall(function()
		create_notification_card(title, text, duration)
	end)
...
function Library:Watermark(name)
	if self._watermark then
		if name ~= nil and self._watermark.SetText then
			self._watermark:SetText(name)
		end
		return self._watermark
	end

	self._watermark = create_watermark(name)
	return self._watermark
end
...
function CompatModule:create_searchbox(settings)
	settings = settings or {}
	return self._section:Searchbox({
		Name = pick(settings, { "title", "Title", "name", "Name" }, "Searchbox"),
		Flag = pick(settings, { "flag", "Flag" }, next_flag("Searchbox")),
		Items = pick(settings, { "options", "Options", "items", "Items" }, {}),
		Default = pick(settings, { "default", "Default", "value", "Value" }, nil),
		Multi = pick(settings, { "multi_dropdown", "MultiDropdown", "multi", "Multi" }, false),
		Callback = pick(settings, { "callback", "Callback" }, function() end),
...
CompatModule.create_label = CompatModule.create_text
CompatModule.create_toggle = CompatModule.create_checkbox
